...
caseManager_comparacion_comercial;
propiedades_materiales;
% Condiciones de borde elegidas para la comparación (temperatura fija sin
% convección ni radiación)
Tin = 22+273; %[K] fija
Tout = -5+273; % [K] fija

dt = 3600*24*2; % Paso temporal de dos días, y se lo deja correr durante
                % cien días para obtener respuesta estacionaria
...
%luego:
% plot(x_centros(i),y_centros(j),'o');
...
% Valor inicial para la iteración de la temperatura en los elementos de la cara superior
if plotT>0
    surfTemp = figure;
...
for iTime=1:3600*24*(diasEstabilizacion+1)/dt %4 días de corrida, 3 para que se estabilice el ciclo de temperaturas. Estudiamos los flujos de calor en el cuarto día.
    ...
    %Ts_norte=0.5*(C1r+C1out+(C2r+C2out)*Tsuperior), Ts_sur=C1in+C2in*Tinferior
...
for i = 2:totNelInLength-1
    for j = 1
        ...
        aux = sparse(totNelInLength,totNelInHeight);
        aux(i,j) = k_yeso*dx/dy(1) + 2*k_yeso*dy(1)/dx + rho_yeso*c_yeso*dy(1)*dx/dt - 2*k_yeso*dx/dy(1); %   -- coeficiente central
        aux(i,j+1) = -k_yeso*dx/dy(1); %            -- coeficiente norte
        aux(i+1,j) = -k_yeso*dy(1)/dx; %            -- coeficiente este
        aux(i-1,j) = -k_yeso*dy(1)/dx; %            -- coeficiente oeste
        aux=aux';
        ...
        A0(nroFila,:)=aux0(:);
        A(nroFila,:)=aux(:);
        B(nroFila)=-2*Tin*k_yeso*dx/dy(1);
    end
end
...
for i = 2:totNelInLength-1
    for j = totNelInHeight
        ...
        aux(i,j) = k_metal*dx/dy(3) + 2*k_metal*dy(3)/dx + rho_metal*c_metal*dy(3)*dx/dt - 2*k_metal*dx/dy(3); %   -- coeficiente central
        aux(i,j-1) = -k_metal*dx/dy(3); %            -- coeficiente sur
        aux(i+1,j) = -k_metal*dy(3)/dx; %            -- coeficiente este
        aux(i-1,j) = -k_metal*dy(3)/dx; %            -- coeficiente oeste
        aux=aux';
        ...
        A0(nroFila,:)=aux0(:);
        A(nroFila,:)=aux(:);
        B(nroFila)=-2*Tout*k_metal*dx/dy(3);
    end
end
...
for i = 1 
    for j = 1
        ...
        aux = sparse(totNelInLength,totNelInHeight);
        aux(i,j) = k_yeso*dx/dy(1) + k_yeso*dy(1)/dx + rho_yeso*c_yeso*dy(1)*dx/dt - 2*k_yeso*dx/dy(1); %   -- coeficiente central
        aux(i,j+1) = -k_yeso*dx/dy(1); %            -- coeficiente norte
        aux(i+1,j) = -k_yeso*dy(1)/dx; %            -- coeficiente este
        aux=aux';
...
for i = 1
    for j = totNelInHeight
        ...
        aux(i,j) = k_metal*dx/dy(3) + k_metal*dy(3)/dx + rho_metal*c_metal*dy(3)*dx/dt - 2*k_metal*dx/dy(3); %   -- coeficiente central
        aux(i,j-1) = -k_metal*dx/dy(3); %            -- coeficiente sur
        aux(i+1,j) = -k_metal*dy(3)/dx; %            -- coeficiente este
        aux=aux';
...
%timestep
TsuperficieExt = Tout*ones(1,length(Tmatriz(end,:)));
if dispDifTemp==1
    disp(['Máxima diferencia de temperatura en cara superior: ' num2str(max(abs(TsuperficieExt-Tmatriz(end,:))))]);
end
TsuperficieInt = Tin*ones(1,length(Tmatriz(1,:)));
T=TIter;


% Matriz de temperaturas ampliada
...
if plotT==2 || (plotT==1 && dia==diasEstabilizacion+1)
    ...
    view([0 0 1]);
%     caxis([15 60]); %este puede o no ir, veremos. para fijar la colorbar
...
QTot_norteMat = [QTot_norteMat;{QTot_norte_2}];
QTot_surMat = [QTot_surMat;{QTot_sur_2}];
