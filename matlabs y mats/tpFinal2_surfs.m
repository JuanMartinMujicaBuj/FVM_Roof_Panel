...
%             bMitadAislanteVec = [bMitadAislanteVec b(1)]; % si pongo b(3) es lo mismo
            ...
            hin=ones(totNelInLength,1)*0.25; %valor inicial, más adelante se recalcula
            ...
            for iTime=1:3600*24*(diasEstabilizacion+1)/dt %4 días de corrida, 3 para que se estabilice el ciclo de temperaturas. Estudiamos los flujos de calor en el cuarto día.
                ...
                if (plotT==2 || (plotT==1 && dia==diasEstabilizacion+1)) &&...
                   ((hora==4 || hora==8 || hora==12 || hora==16 || hora==20 || hora==24) && minutos==0)
                    figure(surfTemp)
                    subplot(2,3,hora/4)
                    surf(XA,YA,TmatrizA-273);
                    shading interp
                    hold on
                    surf(XA,YA,TmatrizA-273,'EdgeAlpha',0.4,'FaceAlpha',0);
                    barra = colorbar;
                    ...
                    zlabel('T ($$^{\circ}$$C)','interpreter','latex','FontSize',7);
                    view([-1 -1 1]);
                    if nroMes==1
                        %caxis([20 38]);
                        cb=colorbar;
